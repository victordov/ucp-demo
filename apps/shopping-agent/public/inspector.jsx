    ...
    <div className="tev" style={{ animationDelay: "0ms" }}>
      ...
      {open && (
        <div className="tev-body">
          <div className="tev-meta" style={{ marginBottom: 8, fontFamily: "var(--font-mono)", fontSize: 10, display: "flex", alignItems: "center", gap: 8 }}>
            <span>{time}</span>
            {ev.hash && <span title="audit chain">#{ev.seq} · {String(ev.hash).slice(0, 10)}…</span>}
            <span style={{ flex: 1 }} />
            <button className="tev-copy" onClick={(e) => { e.stopPropagation(); navigator.clipboard?.writeText(JSON.stringify(ev.payload, null, 2)); e.target.textContent = "copied!"; setTimeout(() => (e.target.textContent = "copy payload"), 1200); }}>copy payload</button>
          </div>
...
function Inspector({ events, onClose }) {
  ...
  React.useEffect(() => {
    if (bodyRef.current && !q) bodyRef.current.scrollTop = bodyRef.current.scrollHeight;
  }, [events.length]);
  ...
  const matches = (e) =>
    ...
    JSON.stringify(e.payload || {}).toLowerCase().includes(ql);
  const shown = events.filter(
    (e) =>
      (tab === "all" || (tab === "ap2" && e.layer === "AP2") || (tab === "ucp" && e.layer === "UCP") || (tab === "pki" && e.layer === "PKI")) &&
      matches(e)
  );
  ...
  return (
    <aside className="inspector">
      ...
      <div className="insp-tabs">
        ...
        <button className={"insp-tab " + (tab === "pki" ? "active" : "")} onClick={() => setTab("pki")}>PKI</button>
      </div>
      <div className="insp-search">
        <input value={q} onChange={(e) => setQ(e.target.value)} placeholder="search events, methods, payloads…" />
        {q && <button onClick={() => setQ("")} title="clear">×</button>}
      </div>
